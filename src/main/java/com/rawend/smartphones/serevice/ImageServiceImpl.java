...
@Override
public Image uplaodImageSmar(MultipartFile file, Long idSmar) throws IOException {
		Smartphone s = new Smartphone();
		s.setIdSmartphone(idSmar);
		return imageRepository.save(Image.builder()
		 .name(file.getOriginalFilename())
		 .type(file.getContentType())
		 .image(file.getBytes())
...
@Override
public List<Image> getImagesParSmar(Long smarId) {
	Smartphone s = smartphoneRepository.findById(smarId).get();
	return s.getImages();
}
}
